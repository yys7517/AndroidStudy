...
public class KwangjuInfo extends AppCompatActivity {
...
// ----------- 에스컬레이터 ---------------------------------------------//
        while(escalator.getText().toString().equals("")) {
            Cursor cursor;
            cursor = db.rawQuery("SELECT * FROM `광주 에스컬레이터` where `역명` = '"+station+"' ",null);
...
// ----------- 엘레베이터 ---------------------------------------------//
        while(elevator.getText().toString().equals("")) {
            Cursor cursor;
            cursor = db.rawQuery("SELECT * FROM `광주 엘리베이터` where `역명` = '"+station+"' ",null);
...
// ----------- 화장실 ---------------------------------------------//
        while(toilet.getText().toString().equals("")) {
            Cursor cursor;
            cursor = db.rawQuery("SELECT * FROM `광주 화장실` where `역명` = '"+station+"' ",null);
...
// ----------- 장애인화장실 ---------------------------------------------//
        while(disabledtoilet.getText().toString().equals("")) {
            Cursor cursor;
            cursor = db.rawQuery("SELECT * FROM `광주 장애인화장실` where `역명` = '"+station+"' ",null);
...
// ----------- 휠체어 리프트 ---------------------------------------------//
        while(wheellift.getText().toString().equals("")) {
            Cursor cursor;
            cursor = db.rawQuery("SELECT * FROM `광주 휠체어리프트` where `역명` = '"+station+"' ",null);
...
// ----------- 고객센터 ---------------------------------------------//
        while(center.getText().toString().equals("")) {
            String rphone = "";
            Cursor cursor;
            cursor = db.rawQuery("SELECT `전화번호` FROM `광주 고객센터` where `역명` = '"+station+"' and `선명` = '"+line+"' ",null);
        ...
        center_phone.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    int permissionCheck = ContextCompat.checkSelfPermission(KwangjuInfo.this, Manifest.permission.CALL_PHONE);
                    if (permissionCheck == PackageManager.PERMISSION_DENIED) {
                        // 권한 없음
                        ActivityCompat.requestPermissions(KwangjuInfo.this,
