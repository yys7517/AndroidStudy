...
//                android.R.layout.simple_list_item_1, list);
        ...
        if (test.equals(line_1)) {
            ...
            Cursor cursor;
            cursor = db.rawQuery("select distinct * from `대전` ", null);
            tname = "";
...
//////////-----------------------------------------------------------------------------------
        ...
        listview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView parent, View v, int position, long id) {
                ...
                Intent it = new Intent(getApplicationContext(), DaejeonInfo.class);   // 인텐트 처리
                it.putExtra("station", product); // 역 이름
                it.putExtra("location",location); // 지역 이름
                it.putExtra("hosun",test); // 호선 명
                startActivity(it);
        ...
        btnSearch.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (a.equals("")) {
                    ...
                    while (flag ) {
                        adapter.clearItem();
                        Cursor cursor = db.rawQuery("SELECT distinct `역명` from `대전` where `역명` LIKE '%"+a+"%'",null);
                        tname = "";
                        ...
                        listview.setAdapter(adapter);
                        Toast.makeText(getApplicationContext()," '"+a+"'(으)로 검색된 대전 '"+test+"'내 역 검색 결과입니다.",Toast.LENGTH_SHORT).show();
                        ...
                        listview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                            ...
                            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                                ...
                                edtStation.setText("");
                                adapter.clearItem();
                                Cursor cursor = db.rawQuery("SELECT distinct `역명` from `대전`",null);
                                tname = "";
