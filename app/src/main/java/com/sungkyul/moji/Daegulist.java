...
import com.sungkyul.moji.info.DaeguInfo;
...
public class Daegulist extends AppCompatActivity {
...
//                android.R.layout.simple_list_item_1, list);
        ...
        if (test.equals(line_1)) {
            ...
            Cursor cursor;
            cursor = db.rawQuery("select distinct * from `대구1호선` ", null);
            tname = "";
            ...
            Cursor cursor;
            cursor = db.rawQuery("select distinct * from `대구2호선` ", null);
            tname = "";
            hosun = "2호선";
            while (cursor.moveToNext()) {
                String name = cursor.getString(0);
            ...
            Cursor cursor;
            cursor = db.rawQuery("select distinct * from `대구3호선` ", null);
            tname = "";
            hosun = "3호선";
            while (cursor.moveToNext()) {
                String name = cursor.getString(0);
...
//////////-----------------------------------------------------------------------------------
        ...
        listview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView parent, View v, int position, long id) {
                ...
                Intent it = new Intent(getApplicationContext(), DaeguInfo.class);   // 인텐트 처리
