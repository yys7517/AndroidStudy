...
public class DaejeonInfo extends AppCompatActivity {
...
// ----------- 에스컬레이터 ---------------------------------------------//
        while(escalator.getText().toString().equals("")) {
            Cursor cursor;
            cursor = db.rawQuery("SELECT * FROM `대전 에스컬레이터` where `역명` = '"+station+"' ",null);
...
// ----------- 엘레베이터 ---------------------------------------------//
        while(elevator.getText().toString().equals("")) {
            Cursor cursor;
            cursor = db.rawQuery("SELECT * FROM `대전 엘리베이터` where `역명` = '"+station+"' ",null);
...
// ----------- 화장실 ---------------------------------------------//
        while(toilet.getText().toString().equals("")) {
            Cursor cursor;
            cursor = db.rawQuery("SELECT * FROM `대전 화장실` where `역명` = '"+station+"' ",null);
...
// ----------- 장애인화장실 ---------------------------------------------//
        while(disabledtoilet.getText().toString().equals("")) {
            Cursor cursor;
            cursor = db.rawQuery("SELECT * FROM `대전 장애인화장실` where `역명` = '"+station+"' ",null);
...
// ----------- 고객센터 ---------------------------------------------//
        while(center.getText().toString().equals("")) {
            String rphone="";
            Cursor cursor;
            cursor = db.rawQuery("SELECT `전화번호` FROM `대전 고객센터` where `역명` = '"+station+"' ",null);
        ...
        center_phone.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    int permissionCheck = ContextCompat.checkSelfPermission(DaejeonInfo.this, Manifest.permission.CALL_PHONE);
                    if (permissionCheck == PackageManager.PERMISSION_DENIED) {
                        // 권한 없음
                        ActivityCompat.requestPermissions(DaejeonInfo.this,
