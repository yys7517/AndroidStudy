...
public class SudokwonInfo extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        center_phone = (TextView)findViewById(R.id.center_phone);
...
// ----------- 에스컬레이터 ---------------------------------------------//
        while(escalator.getText().toString().equals("")) {
            String rspecific="";
            Cursor cursor;
            cursor = db.rawQuery("SELECT * FROM `수도권 에스컬레이터` where `역명` = '"+station+"' ",null);
...
// ----------- 엘레베이터 ---------------------------------------------//
        while(elevator.getText().toString().equals("")) {
            Cursor cursor;
            cursor = db.rawQuery("SELECT * FROM `수도권 엘리베이터` where `역명` = '"+station+"' ",null);
...
// ----------- 무빙워크 ---------------------------------------------//
        while(movingwalk.getText().toString().equals("")) {
            Cursor cursor;
            cursor = db.rawQuery("SELECT * FROM `수도권 무빙워크` where `역명` = '"+station+"' ",null);

            if( cursor.getCount() != 0) {
                movingwalk.setText("O");
            }
            else
...
// ----------- 화장실 ---------------------------------------------//
        while(toilet.getText().toString().equals("")) {
            Cursor cursor;
            cursor = db.rawQuery("SELECT * FROM `수도권 화장실` where `역명` = '"+station+"' ",null);
...
// ----------- 장애인화장실 ---------------------------------------------//
        while(disabledtoilet.getText().toString().equals("")) {
            Cursor cursor;
            cursor = db.rawQuery("SELECT * FROM `수도권 장애인화장실` where `역명` = '"+station+"' ",null);
...
// ----------- 휠체어 리프트 ---------------------------------------------//
        while(wheellift.getText().toString().equals("")) {
            Cursor cursor;
            cursor = db.rawQuery("SELECT * FROM `수도권 휠체어리프트` where `역명` = '"+station+"' ",null);
...
// ----------- 휠체어 충전설비 ---------------------------------------------//
        while(charge.getText().toString().equals("")) {
            Cursor cursor;
            cursor = db.rawQuery("SELECT * FROM `수도권 전동휠체어 충전설비` where `역명` = '"+station+"' ",null);
...
// ----------- 고객센터 ---------------------------------------------//
        while(center.getText().toString().equals("")) {
            String rphone = "";
            Cursor cursor;
            cursor = db.rawQuery("SELECT `전화번호` FROM `수도권 고객센터` where `역명` = '"+station+"' and `선명` = '"+line+"' ",null);
        ...
        center_phone.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    int permissionCheck = ContextCompat.checkSelfPermission(SudokwonInfo.this, Manifest.permission.CALL_PHONE);
                    if (permissionCheck == PackageManager.PERMISSION_DENIED) {
                        // 권한 없음
                        ActivityCompat.requestPermissions(SudokwonInfo.this,
